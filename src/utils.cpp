...
void transform_image(char *image_data, const size_t size, float slope, float intercept, const int max, const int min)
{
    unsigned char *pixels = (unsigned char *)image_data;
    float buf[size];
    float ymin;
    float ymax;

    // Min - Max clapping and
    ...
    for (size_t i = 0; i < size; i++)
    {
        if (pixels[i] > max)
            ...
            pixels[i] = min;
        buf[i] = (slope * ((pixels[i])) + intercept);
    }

    // Calculating current minimum and maximum
    ymin = buf[0];
    ymax = buf[0];
    for (size_t i = 0; i < size; i++)
    {
    ...
    // Final transform
    for (size_t i = 0; i < size; i++)
        pixels[i] = ((buf[i] - ymin) / (ymax - ymin)) * 255;
}
