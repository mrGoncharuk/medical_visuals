...
TextRenderer::TextRenderer(/* args */): vbo(0), vao(0), programID(0)
{
}


TextRenderer::TextRenderer(int winWidth, int winHeight): vbo(0), vao(0), programID(0), windowWidth(winWidth), windowHeight(winHeight)
{
}
...
void		TextRenderer::createShaderProgram(const std::string &vertexShaderPath, const std::string &fragmentShaderPath)
{
...
void		TextRenderer::loadFont(const std::string &fontPath)
{
    ...
    glUniformMatrix4fv(glGetUniformLocation(this->programID, "projection"), 1, GL_FALSE, glm::value_ptr(projection));
    // FreeType
    // --------
    FT_Library ft;
